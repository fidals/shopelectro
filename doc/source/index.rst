...
Для восстановления базы данных и медиафайлов достаточно запустить ``make restore`` - специальный скрипт скачает последний бекап с сервера (всё опционально, можно выбрать что нужно) и разместит данные директориях из которых их забирает production-версия контейнеров, т.е.:
...
N.B.: некоторые данные (например, медиафайлы) могут иметь большой размер (на момент написания этой заметки, архив с медиафайлами Shopelectro весил ~4GB), поэтому нужно учесть доступное место на жестком диске и, при необходимости, освободить место под бекапы.
