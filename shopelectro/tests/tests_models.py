...
class ProductModel(TestCase):
    ...
    def test_get_products_by_tags(self):
        ...
        fst_tag, snd_tag = Tag.objects.first(), Tag.objects.last()
        products = Product.objects.get_by_tags([fst_tag.id, snd_tag.id])
        test_products = fst_tag.products.all() & snd_tag.products.all()
